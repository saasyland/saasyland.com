...
describe("proxy helper", () => {
  it("redirects auth callback without session to sign-in", async () => {
    ...
    expect(response.headers.get("location")).toContain(CONSTANTS.ROUTES.SIGN_IN)
  })

  it("redirects auth callback with session to post-auth destination", async () => {
    expect.hasAssertions()
    resetProxyMocks()
    getSessionMock.mockResolvedValue(createSession(CONSTANTS.PERMISSIONS.ROLES.ADMIN))

    const response = await proxy(new NextRequest(createTestRequestUrl("/auth/callback")))

    expect(response.headers.get("location")).toContain(CONSTANTS.ROUTES.ADMIN)
  })
  ...
  it("redirects non-admin users away from admin routes", async () => {
    expect.hasAssertions()
    resetProxyMocks()
    getSessionMock.mockResolvedValue(createSession(CONSTANTS.PERMISSIONS.ROLES.CUSTOMER))

    const response = await proxy(new NextRequest(createTestRequestUrl("/admin")))

    expect(response.headers.get("location")).toContain(CONSTANTS.ROUTES.APP)
  })
