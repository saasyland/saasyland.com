...
import { getCategory } from "~/src/modules/category/use-cases/get-category.use-case"
...
describe("get-category", () => {
  it("returns a category for admins", async () => {
    ...
    await expect(getCategory(CATEGORY_ID)).resolves.toStrictEqual(dbMocks.dbRow)
  ...
  it("rejects with not-found when the category does not exist", async () => {
    ...
    await expect(getCategory(CATEGORY_ID)).rejects.toMatchObject({ digest: "NEXT_HTTP_ERROR_FALLBACK;404" })
  })
  ...
  it("rejects when the caller is not an admin", async () => {
    ...
    await expect(getCategory(CATEGORY_ID)).rejects.toMatchObject({ digest: "NEXT_HTTP_ERROR_FALLBACK;403" })
  })
  ...
  it("rejects when the caller is signed out", async () => {
    ...
    await expect(getCategory(CATEGORY_ID)).rejects.toMatchObject({ digest: "NEXT_HTTP_ERROR_FALLBACK;401" })
  })
})
