...
import "~/src/integrations/better-auth/auth._server"
import type * as RedisConfig from "~/src/integrations/redis/redis.config"
...
const redisDelMock = vi.hoisted(() => vi.fn<Redis["del"]>())
const redisExpireMock = vi.hoisted(() => vi.fn<Redis["expire"]>())
const redisGetMock = vi.hoisted(() => vi.fn<Redis["get"]>())
const redisGetdelMock = vi.hoisted(() => vi.fn<Redis["getdel"]>())
const redisIncrMock = vi.hoisted(() => vi.fn<Redis["incr"]>())
const redisSetMock = vi.hoisted(() => vi.fn<Redis["set"]>())

const capturedConfig = vi.hoisted(() => ({ value: undefined as BetterAuthOptions | undefined }))

vi.mock(import("server-only"), () => ({}))

vi.mock(import("~/src/integrations/redis/redis.config"), async (importOriginal): Promise<Partial<typeof RedisConfig>> => {
