...
it("escapes structured data so post titles cannot close the script element", () => {
  const output = buildPostStructuredDataHtml({ ...BASE_INPUT, title: "</script><script>alert(1)</script>" })
  const { __html: html } = output
  expect(html).not.toContain("<")
  expect(JSON.parse(html)).toEqual(buildPostStructuredData({ ...BASE_INPUT, title: "</script><script>alert(1)</script>" }))
})

it("estimates reading time for Japanese prose without spaces", () => {
  const content = "認証と請求の仕組みを学びます。".repeat(100)
  expect(readingTimeMinutes({ contents: [{ content }] }, "ja-JP")).toBeGreaterThan(1)
})
