...
import { DataTable, type DataTableProps } from "~/src/presentation/components/custom/data-table/data-table"
import type { DataTableFeatures, DataTableOptions } from "~/src/presentation/components/custom/data-table/features"
...
type TableQuery = NonNullable<DataTableProps<Person>["query"]>
  ...
  const index = createRoute({
    component: () => (
      <DataTable
        ...
        options={{ initialState: { pagination: { pageIndex: 0, pageSize: 2 } }, ...options }}
      />
    ),
    getParentRoute: () => root,
    path: "/",
...
describe("data table server queries", () => {
  ...
  it("surfaces request failures through the route boundary and retries", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: messages.errors.global.retry }))
    expect(await screen.findByText("Grace")).toBeVisible()
    await waitFor(() => {
