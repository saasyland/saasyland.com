...
describe("list-users", () => {
  it("lists users for admins", async () => {
    ...
    await expect(getUsers()).resolves.toStrictEqual([dbMocks.dbRow])
  ...
  it("rejects when the caller is not an admin", async () => {
    ...
    await expect(getUsers()).rejects.toMatchObject({ digest: "NEXT_HTTP_ERROR_FALLBACK;403" })
  })
  ...
  it("rejects when the caller is signed out", async () => {
    ...
    await expect(getUsers()).rejects.toMatchObject({ digest: "NEXT_HTTP_ERROR_FALLBACK;401" })
  })
})
