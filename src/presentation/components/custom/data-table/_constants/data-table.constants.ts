...
export const DATA_TABLE = {
  CLASSES: {
    ...
    DENSITY: {
      ...
      default:
        "[&_td:not([data-system-column])]:px-3.5 [&_td:not([data-system-column])]:py-3 [&_th:not([data-system-column])]:px-3.5 [&_th:not([data-system-column])]:py-2.5 [&_td[data-system-column]]:!p-0 [&_th[data-system-column]]:!p-0",
    },
    EMPTY_CELL: "h-24 text-center text-muted-foreground",
    LAYOUT: {
      BODY: "[&_tr]:group [&_tr]:transition-colors [&_tr]:hover:bg-secondary/20 [&_tr[data-state=selected]]:hover:bg-secondary/50",
      ...
      /** Bottom edge lives on `th` (same color as column dividers) — not on `tr`, avoids double lines and corner dots. */
      HEADER: "[&_tr]:border-0 [&_tr]:hover:bg-transparent",
      /**
       ...
       */
      HEAD_CONTENT:
        "border-b border-r border-border/40 bg-muted last:border-r-0 hover:bg-secondary/60 data-[sorted]:bg-primary/20 data-[sorted]:hover:bg-primary/30",
      PAGINATION: "shrink-0 border-t border-border/40 px-2.5 py-3",
      /**
      ...
      TABLE:
        "table-fixed border-collapse border-spacing-0 [&_th]:text-xs [&_th]:font-medium [&_th]:tracking-wider [&_th]:text-muted-foreground [&_th]:uppercase",
      /**
      ...
      TOOLBAR: "flex shrink-0 flex-wrap items-center gap-3 border-b border-border/40 px-2.5 py-3",
      TOOLBAR_ACTIONS: "flex shrink-0 flex-wrap items-center gap-3 sm:ml-auto",
      /** Fixed width (icon + longest default label) — no layout shift between states. */
      TOOLBAR_FETCH: "w-[9.5rem] shrink-0 justify-start",
      /** Collapsible filter chips row — same surface as the main toolbar (`bg-card` via root). */
      TOOLBAR_FILTERS_BAR: "flex shrink-0 flex-wrap items-center gap-3 border-b border-border/40 px-2.5 py-3",
    },
  ...
  /** Skeleton rows while toolbar fetch is in flight (override via `toolbar.fetch.pendingRows`). */
  FETCH_PENDING_ROWS: 5,
  PAGINATION: {
