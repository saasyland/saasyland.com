...
**A coverage gate rather than a coverage badge.** 176 test files, 651 tests, and four thresholds in the config:
...
**Clear ownership and typed checks.** The public reference app groups operations by feature and configures Vite+ to check both lint rules and types:
...
Route files coordinate navigation; feature use cases live in `src/modules`; vendor setup lives in `src/integrations`; UI lives in `src/presentation`. Server-only modules keep Worker bindings out of the client. Ownership within those folders is a convention to review, not an import restriction enforced by this snippet.

**Strictness with no escape hatch.** `strict`, plus `exactOptionalPropertyTypes`, `noUncheckedIndexedAccess`, `noPropertyAccessFromIndexSignature` and `verbatimModuleSyntax`. `typescript/no-explicit-any` is an error, and the lint categories are configured as errors, so the check has a failing exit code.
