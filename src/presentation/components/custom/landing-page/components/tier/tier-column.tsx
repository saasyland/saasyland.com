...
import { TierBand } from "~/src/presentation/components/custom/landing-page/components/tier/tier-band"
import { TierFeature } from "~/src/presentation/components/custom/landing-page/components/tier/tier-feature"
...
export const TierColumn = ({ mostPopular, tier }: Readonly<{ mostPopular: string; tier: Tier }>): JSX.Element => (
  <HighlightItem
    className={cn("border-border not-last:border-b md:not-last:border-r md:not-last:border-b-0", {
      "bg-background": !tier.isFeatured,
      "bg-card": tier.isFeatured,
    })}
      ...
      <div className="mt-auto pt-10 md:pt-12">
        <Link
          className={cn(
            ...
            {
              "bg-primary text-primary-foreground hover:bg-primary/88": tier.isFeatured,
              "border border-border text-foreground hover:bg-muted": !tier.isFeatured,
            },
          )}
          to={ROUTES.SIGN_UP}
          search={{ tier: tier.id }}
