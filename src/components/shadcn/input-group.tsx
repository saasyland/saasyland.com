...
const inputGroupAddonVariants = cva(
  "flex h-auto cursor-text select-none items-center justify-center gap-2 py-1.5 font-medium text-muted-foreground text-xs group-data-[disabled=true]/input-group:opacity-50 [&>kbd]:rounded-lg [&>svg:not([class*='size-'])]:size-4",
  {
    variants: {
      align: {
...
function InputGroupAddon({
  ...
  align = "inline-start",
  ...props
}: ComponentProps<"div"> & VariantProps<typeof inputGroupAddonVariants>): JSX.Element {
...
function InputGroupText({ className, ...props }: ComponentProps<"span">): JSX.Element {
  return (
    <span
      className={cn(
...
function InputGroupInput({ className, ...props }: ComponentProps<"input">): JSX.Element {
  return (
    <Input
...
function InputGroupTextarea({ className, ...props }: ComponentProps<"textarea">): JSX.Element {
  return (
    <Textarea
      data-slot="input-group-control"
      className={cn(
