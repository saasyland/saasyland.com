...
import { createAuthActionUrl, createCookieHeader, createTestRequestUrl, TEST_APP_URL } from "~/tests/helpers/test-request"
...
describe("redirect pathname component", () => {
  it("preserves explicit default-locale prefixes in redirects", () => {
    expect.hasAssertions()
    expect(redirectPathname("/en/admin", CONSTANTS.ROUTES.SIGN_IN)).toBe("/en/auth/sign-in")
    expect(redirectPathname("/admin", CONSTANTS.ROUTES.SIGN_IN)).toBe(CONSTANTS.ROUTES.SIGN_IN)
  })
  it("localizes redirects for non-default locales", () => {
    expect.hasAssertions()
    expect(redirectPathname("/pl/admin", CONSTANTS.ROUTES.APP)).toBe("/pl/app")
  })
})
